...
public class Aggregate extends Operator {
	...
	// some code goes here
    	...
    	Type t = child.getTupleDesc().getType(afield);
    	if (gfield == -1)
    		noGrouping = true;
    	if (t == Type.INT_TYPE)
    	{
    		aggre = new IntAggregator(gfield, gbFieldType, afield, aop);
    	}
	...
	// some code goes here
    	...
    	if (noGrouping)
    	{
    		...
    		fieldType[0] = this.child.getTupleDesc().getType(aField);
    	...
    	else
    	{
    		...
    		fieldType[0] = child.getTupleDesc().getType(gField);
    		fieldName[0] = child.getTupleDesc().getFieldName(gField);
    		
    		fieldType[1] = this.child.getTupleDesc().getType(aField);
